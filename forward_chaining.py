class Rule:
    ...
    def follows(self, facts): # tìm ra cái facts thiếu

        for fact in self.left:# lấy từng fact trong vế trái
            if fact not in facts: # nếu không thuộc list facts tổng thì trả về 
                return fact
        return None # trả về none (nghĩa là không thiếu) khi mà vế trái đủ điều kiện cho vế phải
...
class ForwardChaining:
    ...
    def __init__(self, rule,fact,goal, file_name):
        ...
        # rules, facts, goal = self.read_data(file_name) # lấy luật , triệu chứng và bệnh
    ...
    def forward_chaining(self, rules, facts, goal):
        ...
        # while goal not in facts: # khi bệnh chưa nằm trong facts tìm thấy
        while 1:
            rule_applied = False
            iteration += 1
            ...
            for rule in rules:
                ...
                if rule.flag2: # cập nhật trùng
                    self.output += "bỏ qua, vì flag2 đã được cập nhật.\n"
                    continue

                if rule.right in facts:# nếu vế phải đã được cm rồi#bệnh có rồi nhưng vế trái tm nhưng bệnh có rùi
                ...
                if missing is None:# cập nhật cờ 1, tức là vế trái đủ để kết luận vế phải
                    rule_applied = True
                    rule.flag1 = True
                    ...
                    break
                else: # do vế trái thiếu mất fact# đưa ra thiếu
